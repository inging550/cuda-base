...
int main()
{
  ...
  d_img.upload(h_img);
  // 平移
  cv::Mat transmat = (cv::Mat_<double>(2,3)<<1,0,70,0,1,50);
  cv::cuda::warpAffine(d_img, d_result1, transmat, h_img.size());
  // 旋转
  cv::Point2f pt(d_img.cols/2, d_img.rows/2);
  cv::Mat rotmat = cv::getRotationMatrix2D(pt, 45, 1);
  cv::cuda::warpAffine(d_img, d_result2, rotmat, d_img.size());
  // return
  d_result1.download(h_result1);
  d_result2.download(h_result2);
  cv::imshow("1", h_result1);
  cv::imshow("2", h_result2);
  cv::waitKey(0);
  return 0;
}